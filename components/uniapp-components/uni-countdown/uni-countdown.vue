		...
		methods: {
			...
			countDown() {
				...
				if (seconds > 0) {
					...
					// hour = parseInt(seconds / 1000 / 60 / 60, 10); //计算剩余的小时
					// minute = parseInt(seconds / 1000 / 60 % 60, 10);//计算剩余的分钟
					// second  = parseInt(seconds / 1000 % 60, 10);//计算剩余的秒数
				} else {
					this.timeUp()
				}
                if (day < 10) {
				...
				// console.log("内部的打印: " + this.day, this.hour, this.minute, this.second);
                this.d = day
				this.h = hour
				this.i = minute
				this.s = second
